...
<body>
    ...
    <table class="customer-info">
        ...
        <tr>
            ...
            <td class="value">{{ strtoupper($order->type) }}</td>
        </tr>
        <tr>
            <td class="label">Mobile:</td>
            <td class="value">{{ $user->mobile }}</td>
            <td class="label">Amount:</td>
            <td class="value">₹ {{ number_format((float)$order->amount, 2) }}</td>
        </tr>
    </table>
        ...
        <tbody>
            @foreach($order_items as $index => $item)
                <tr>
                    ...
                    <td>{{ $item->product_name }}<br>Part No: {{ $item->product->product_code }}<br>{{ $item->product->remarks }}</td>
